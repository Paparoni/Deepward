const EVENT_HANDLERS = {
  battle(state) {
    ...
    Engine.log(
      state,
      `You round a corner and ${monsters.length > 1 ? 'a pack of foes' : 'a lone foe'} blocks the path: <b>${monsters.map((m) => m.name).join(', ')}</b>.`,
      'flavor',
    );
  ...
  merchant(state) {
    ...
    Engine.log(
      state,
      `A hooded merchant has set up a small stall between the pillars. "Something for the journey?"`,
      'flavor',
    );
  ...
  gamble(state) {
    Engine.log(
      state,
      `A grinning figure crouches over a set of worn bone dice. "Wager some gold?"`,
      'flavor',
    );
    ...
    choices.push({
      label: 'Walk away',
      act: (s) => {
        Engine.log(s, 'You keep your coin and keep moving.', 'flavor');
        Engine.finishRoom(s);
      },
    });
  ...
  shrine(state) {
    ...
    Engine.setChoices(state, [
      {
        label: 'Rest at the shrine (restore HP & MP)',
        act: (s) => {
      ...
      {
        label: 'Leave an offering (small gold cost, minor blessing)',
        act: (s) => {
          ...
          Engine.log(
            s,
            `You leave ${cost} gold. Something unseen nods in approval. Partial healing received.`,
            'good',
          );
          Engine.finishRoom(s);
        },
  ...
  mystery(state) {
    const flavors = [
      {
        t: 'Faded murals line the walls, depicting a war no living scholar remembers.',
        choice: 'Study the murals',
        reward: (s) => {
          s.player.gold += U.randInt(3, 10);
        },
        res: 'You notice a few coins wedged in a crack.',
      },
      {
        t: 'A cold draft carries a sound like distant singing.',
      ...
      },
      {
        ...
        choice: 'Read the journal',
        reward: (s) => {
          s.player.xp += Math.round(BALANCE.xpToNext(s.player.level) * 0.05);
          Engine.checkLevelUp(s);
        },
        res: 'The writing teaches you something useful. Minor experience gained.',
      },
    ];
    ...
    Engine.setChoices(state, [
      ...
      },
      {
        ...
        act: (s) => {
          Engine.log(s, 'Curiosity can wait. You press onward.', 'flavor');
          Engine.finishRoom(s);
        },
      },
    ]);
    ...
    Engine.setChoices(state, [
      {
        ...
        act: (s) => {
          ...
          for (const m of monsters) {
            ...
            m.goldDrop = Math.round(m.goldDrop * 1.35);
            m.xpDrop = Math.round(m.xpDrop * 1.35);
          }
          Engine.log(
            s,
  ...
  archive(state) {
    ...
    Engine.setChoices(state, [
      {
        label: 'Study the surviving texts (uses time, grants XP)',
        act: (s) => {
      ...
      {
        label: 'Search for a hidden cache instead',
        act: (s) => {
          if (Math.random() < 0.5) {
  ...
  wishing_well(state) {
    Engine.log(
      state,
      `A well sinks into darkness, coins glinting faintly on its rim. It feels like it is owed something.`,
      'flavor',
    );
    Engine.setChoices(state, [
      {
        label: 'Toss in a coin (10 gold, uncertain fortune)',
        act: (s) => {
          if (s.player.gold < 10) {
            Engine.log(s, "You don't have enough gold for that.", 'bad');
            Engine.renderCurrentChoices(s);
      ...
      {
        label: 'Leave the well undisturbed',
        act: (s) => {
  ...
  collapse(state) {
    ...
    Engine.setChoices(state, [
      {
        label: 'Squeeze through the gap (uses SPD, risk of injury)',
        act: (s) => {
      ...
      {
        label: 'Take the long way around (safe, no risk)',
        act: (s) => {
  ...
  wandering_healer(state) {
    Engine.log(
      state,
      `A robed figure tends a small fire, humming something old. "Wounds, traveler? I can mend what's mendable."`,
      'flavor',
    );
    const cost = Math.round(12 + state.dungeon.dungeonLevel * 1.5);
    Engine.setChoices(state, [
      {
        label: `Pay ${cost} gold for a full restoration`,
        act: (s) => {
      ...
      {
        label: 'Accept a free, partial blessing instead',
        act: (s) => {
          const heal = Math.round(s.derived.maxHp * 0.2);
          s.player.hp = Math.min(s.derived.maxHp, s.player.hp + heal);
          Engine.log(
            s,
            `"On the house." A small warmth spreads through you. Recovered <b>${heal} HP</b>.`,
            'good',
          );
          Engine.finishRoom(s);
        },
  ...
  cursed_altar(state) {
    ...
    Engine.setChoices(state, [
      {
        label: 'Make the pact (+30% ATK & MATK, -15% max HP for the rest of this dungeon)',
        act: (s) => {
          if (s.dungeon._altarPact) {
            Engine.log(s, `The altar has nothing left to offer you.`, 'flavor');
            Engine.finishRoom(s);
            return;
          }
      ...
      },
      {
        ...
        act: (s) => {
          Engine.log(
            s,
            `Some power isn't worth what it asks. You leave the altar cold and silent.`,
            'flavor',
          );
          Engine.finishRoom(s);
        },
  ...
  bonfire(state) {
    ...
    const boosts = [
      {
        stat: 'atk',
        flat: Math.round(3 + dlvl * 0.6),
        label: 'Sharpen your blade',
      },
      {
        stat: 'def',
        flat: Math.round(3 + dlvl * 0.6),
        label: 'Reinforce your guard',
      },
      {
        stat: 'spd',
        flat: Math.round(2 + dlvl * 0.4),
        label: 'Stretch and steady your footing',
      },
    ];
    const choices = boosts.map((b) => ({
      ...
      act: (s) => {
        ...
        Engine.log(
          s,
          `The preparation settles into your bones. <b>+${b.flat} ${STAT_BY_ID[b.stat].short}</b> for the rest of this dungeon.`,
          'good',
        );
        Engine.finishRoom(s);
      },
    ...
    choices.push({
      label: 'Rest instead (restore some HP & MP)',
      act: (s) => {
        ...
        Engine.log(
          s,
          `You rest a while by the embers. Recovered <b>${healHp} HP</b> and <b>${healMp} MP</b>.`,
          'good',
        );
        Engine.finishRoom(s);
      },
  ...
  puzzle_door(state) {
    Engine.log(
      state,
      `An ancient lock covered in shifting runes bars the way forward. The pattern feels almost readable.`,
      'flavor',
    );
    Engine.setChoices(state, [
      {
        label: 'Study the pattern (uses HIT EFF)',
        act: (s) => {
          const chance = U.clamp(35 + s.derived.hitEff * 1.1, 20, 90);
          if (Math.random() * 100 < chance) {
            const item = Generators.generateItem(s.dungeon.dungeonLevel, {
              lootBonus: s.dungeon.difficulty.lootBonus,
              forcedMinTier: 'uncommon',
              affinities: Engine.lootAffinities(s),
            });
            Engine.log(s, `The runes click into place. A hidden compartment slides open.`, 'good');
            Engine.presentItemDrop(s, item, (s2) => {
            ...
            Engine.log(
              s,
              `<b>${monsters.map((m) => m.name).join(', ')}</b> spill from a concealed alcove.`,
              'bad',
            );
            Engine.startCombat(s, monsters);
          }
      ...
      {
        label: 'Force it open (guaranteed, minor damage)',
        act: (s) => {
  ...
  guarded_cache(state) {
    ...
    for (const m of monsters) {
      ...
      m.mdef = Math.round(m.mdef * 1.15);
      m.hp = Math.round(m.hp * 1.15);
      m.maxHp = m.hp;
    }
    Engine.log(
      state,
    ...
    Engine.setChoices(state, [
      {
        ...
        act: (s) => {
          ...
          if (roll < 0.35) {
            ...
            Engine.log(
              s,
              `The water is impossibly cold and impossibly clean. You recover <b>${heal} HP</b>.`,
              'good',
            );
          } else if (roll < 0.55) {
            ...
            s.dungeon._buffs = s.dungeon._buffs || [];
            s.dungeon._buffs.push({
              stat,
              flat,
              name: 'Strength of the Deep Pool',
            });
            Engine.refreshDerived(s);
            Engine.log(
              s,
              `Strength you don't recognize floods your limbs. <b>+${flat} ${STAT_BY_ID[stat].short}</b> for the rest of this dungeon.`,
              'good',
            );
          } else {
      ...
      {
        label: "Search the pool's edge instead",
        act: (s) => {
          if (Math.random() < 0.5) {
            const gold = U.randInt(15, 40);
            s.player.gold += gold;
  ...
  abandoned_camp(state) {
    Engine.log(
      state,
      `The ashes of a cold campfire surround a pack that its owner never came back for.`,
      'flavor',
    );
    Engine.setChoices(state, [
      {
        label: "Search the delver's pack (find their gear)",
        act: (s) => {
          const item = Generators.generateItem(s.dungeon.dungeonLevel, {
            lootBonus: s.dungeon.difficulty.lootBonus,
            forcedMinTier: Math.random() < 0.5 ? 'common' : 'uncommon',
            affinities: Engine.lootAffinities(s),
          });
      ...
      {
        label: 'Take supplies instead (small heal + material)',
        act: (s) => {
