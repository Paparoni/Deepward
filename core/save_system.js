const SaveSystem = {
  ...
  snapshot(state) {
    ...
    delete player._revivedThisDungeon;
    delete player._revivedThisFight;
    delete player._stunned;
    return {
      game: 'DEEPWARD',
  ...
  validate(data) {
    if (!data || data.game !== 'DEEPWARD' || !Number.isInteger(data.version))
      throw new Error('This is not a Deepward save file.');
    if (data.version > this.version) throw new Error('This save was created by a newer version of Deepward.');
    if (!data.player || !CLASS_BY_ID[data.player.classId])
      throw new Error('The save contains an unknown character class.');
    if (!data.equipment || !Array.isArray(data.inventory))
      throw new Error('The save is missing equipment or inventory data.');
    return data;
  },
  ...
  save(state) {
    try {
      ...
      console.error('Deepward save failed:', error);
      return false;
    }
  },
  ...
  restore(data) {
    ...
    const cleanItem = (item) => {
      ...
      return {
        ...
        slotLabel: cleanText(item.slotLabel),
        stats: Object.fromEntries(
          Object.entries(item.stats || {})
            .filter(([id, value]) => STAT_BY_ID[id] && Number.isFinite(Number(value)))
            .map(([id, value]) => [id, Number(value)]),
        ),
        uniqueTraits: (Array.isArray(item.uniqueTraits) ? item.uniqueTraits : []).map((t) => ({
          ...t,
          name: cleanText(t.name),
          desc: cleanText(t.desc),
        })),
        mythicTrait: item.mythicTrait
          ? {
              ...item.mythicTrait,
              name: cleanText(item.mythicTrait.name),
              desc: cleanText(item.mythicTrait.desc),
            }
          : null,
      };
    };
    ...
    const state = {
      ...
      log: [],
      settings: {
        combatPace: ['fast', 'normal', 'cinematic'].includes(data.settings?.combatPace)
          ? data.settings.combatPace
          : 'normal',
  ...
  loadLocal() {
    try {
      this.restore(this.readLocal());
    } catch (error) {
      alert(`Could not load save: ${error.message}`);
    }
  },
  ...
  export(state) {
    try {
      ...
      link.href = url;
      link.download = `deepward-${state.player.name.replace(/[^a-z0-9_-]+/gi, '_')}-lvl${state.player.level}.json`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
    } catch (error) {
      alert(`Could not export save: ${error.message}`);
    }
  },
  ...
  importFile(file) {
    ...
    reader.onload = () => {
      try {
        this.restore(JSON.parse(reader.result));
      } catch (error) {
