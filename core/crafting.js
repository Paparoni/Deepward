/* ============================================================
   SOULFORGE CRAFTING
   Materials are enemy drops; recipes are rare boss discoveries. Crafted mythics
   are the only source of the fixed traits in CRAFTED_MYTHIC_TRAITS.
   ============================================================ */
const CRAFTING_MATERIALS = [
  ['void_amber','Void Amber'], ['wyrm_scale','Wyrm Scale'], ['starsteel','Starsteel Ingot'], ['ember_heart','Ember Heart'],
  ['frost_bloom','Frost Bloom'], ['storm_core','Storm Core'], ['venom_gland','Venom Gland'], ['sunshard','Sunshard'],
  ['grave_ash','Grave Ash'], ['titan_bone','Titan Bone'], ['moon_silk','Moon Silk'], ['obsidian','Obsidian Shard'],
...
const MYTHIC_RECIPES = [
  {id:'worldsplitter', name:'Worldsplitter', slot:'weapon', element:'physical', mythicTrait:'soulforge_fury', uniqueTraits:['crit_dmg','giant_slayer'], requirements:{starsteel:6,titan_bone:3,void_amber:2}, stats:{atk:29,def:9,hitEff:9,physicalDmg:14,hitRes:8}},
  {id:'dawnkeep', name:'Dawnkeep Visor', slot:'helmet', element:'holy', mythicTrait:'dawnward', uniqueTraits:['regen','thorns'], requirements:{sunshard:6,celestial_ore:4,ancient_bark:2}, stats:{def:19,mdef:15,hitRes:10,holyDmg:15,spd:5}},
  {id:'stormveil', name:'Stormveil Mantle', slot:'chest', element:'lightning', mythicTrait:'tempest_step', uniqueTraits:['dodge','crit_chance'], requirements:{storm_core:5,moon_silk:4,runic_circuit:3}, stats:{def:18,mdef:16,spd:11,lightningDmg:16,hitEff:7}},
  {id:'mourning', name:'Mourning Covenant', slot:'accessory1', element:'dark', mythicTrait:'grave_pact', uniqueTraits:['lifesteal','mana_eff'], requirements:{grave_ash:6,shadow_thread:4,void_amber:2}, stats:{matk:24,mdef:12,darkDmg:18,hitRes:8,spd:6}},
  {id:'basilisk', name:'Basilisk Signet', slot:'accessory1', element:'poison', mythicTrait:'plague_crown', uniqueTraits:['stun_chance','double_hit'], requirements:{venom_gland:6,black_lotus:3,gilded_fang:3}, stats:{atk:20,matk:20,poisonDmg:19,hitEff:10,spd:7}},
  {id:'phoenix', name:'Phoenix-Heart Greaves', slot:'legs', element:'fire', mythicTrait:'phoenix_ward', uniqueTraits:['regen','vengeful'], requirements:{phoenix_feather:4,ember_heart:6,ashglass:3}, stats:{def:17,spd:12,fireDmg:18,atk:14,mdef:10}},
  {id:'riftsong', name:'Riftsong Blade', slot:'weapon', element:'ice', mythicTrait:'riftsong', uniqueTraits:['crit_chance','piercing'], requirements:{rift_splinter:5,echo_crystal:4,frost_bloom:5}, stats:{atk:27,matk:18,iceDmg:19,spd:9,hitEff:9}},
...
const Crafting = {
  canCraft(state, recipe){
    return state.player.recipes.includes(recipe.id) && Object.entries(recipe.requirements)
      .every(([material,count])=>(state.player.materials[material]||0)>=count);
  },
  ...
  createItem(recipe, level){
    ...
    return {
      uid:U.uid(), name:recipe.name, slot:recipe.slot, slotLabel:slot.label,
      tier:'mythic_legendary', ilvl:level, crafted:true, element:recipe.element,
      stats:Object.fromEntries(Object.entries(recipe.stats).map(([stat,value])=>[stat,scaleStat(stat,value)])),
  ...
  craft(state, recipeId){
    ...
    for(const [material,count] of Object.entries(recipe.requirements)) state.player.materials[material]-=count;
    const item = this.createItem(recipe, Math.max(1,state.player.level));
    Engine.grantItem(state,item);
    Engine.log(state, `The Soulforge creates <b style="color:${TIER_BY_ID[item.tier].color}">${item.name}</b>.`, 'good');
    return true;
  },
};
